			...
			<div class="divide-y">
				...
				>
					<div
						...
						class="h-12 flex"
					>
						<div class="bg-gray-500 w-48 px-4 h-full flex items-center">
							{{ skill.name }}
						</div>
						<div class="h-full relative bg-gray-600 w-full">
							<div class="absolute w-12 h-12 bg-slate-500 top-0" />
						</div>
					</div>
				</div>
			</div>
...
export default defineComponent({
	...
	setup() {
		...
		const getAvailableMitigationForTime = (time: number) => {
			const available: Record<string, MitigationSkill[]> = Object.entries(groupsMitigation.value).reduce((object, [memberId, memberMitigationSkills]) => {
